...
var fs = require('fs');
var defaultCode = fs.readFileSync('./lib/models/defaultCode.txt');

var contestantSchema = new Schema({
  name: { type: String, unique: true },
  code: { type: String, required: true, default: defaultCode },
  personName: { type: String },
  personEmail: { type: String },
  personSemester: { type: String },
  personProgram: { type: String },
...
contestantSchema.statics.findHighscores = function(cb) {
  this.find().sort({wins:-1, name: 1}).exec(cb);
}
...
contestantSchema.statics.findAll = function(cb) {
  this.find({}).sort().exec(cb);
};

contestantSchema.statics.recalcAllScores = function(cb) {
  var that = this;
  this.findAll(function(err, contestants) {
    if (err) {
      return cb(err);
    }
    async.eachSeries(contestants, function(contestant, cb) {
      contestant.recalcScores(cb);
    }, function(err) {
      return cb(err);
    });
  });
...
contestantSchema.methods.recalcScores = function(cb) {
  var that = this;
  Result.getWinCountForPlayer(that._id, function(err, loadedWins) {
    that.wins = loadedWins;
    Result.getResultCountForPlayer(that._id, function(err, loadedResults) {
      that.results = loadedResults;
      that.save(cb);
    });
  });
}
