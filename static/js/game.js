...
var Runner = function(id, game, cfg) {
...
Runner.prototype.start = function() { // game instance should call runner.start() when its finished initializing and is ready to start the game loop
...
Runner.prototype.resetStats = function() {
	this.stats = {
		count:  0,
		fps:    0,
		update: 0,
...
Runner.prototype.updateStats = function(update, draw) {
	if (this.cfg.stats) {
...
Runner.prototype.drawStats = function(ctx) {
	if (this.cfg.stats) {
	...
	Runner.prototype.hideCursor = function() { this.canvas.style.cursor = 'none'; };
	Runner.prototype.showCursor = function() { this.canvas.style.cursor = 'auto'; };

	Runner.prototype.alert= function(msg) {
...
Runner.prototype.confirm = function(msg) {
	this.stop(); // alert blocks thread, so need to stop game loop in order to avoid sending huge dt values to next update
...
Game = {
	...
	start: function(id, game, cfg) {
		if (Game.compatible()) {
			var runner = new Game.Runner(id, game, cfg);// return the game instance, not the runner (caller can always get at the runner via game.runner)
			return runner.game;
		}
	},
